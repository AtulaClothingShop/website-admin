        ...
        <div className='dropdown-menu-right dropdown-menu-lg overflow-hidden p-0'>
          <List className='text-left bg-transparent d-flex align-items-center flex-column pt-0'>
            ...
            <div className='pl-3  pr-3'>
              <div className='font-weight-bold text-center pt-2 line-height-1'>{currentUser?.fullname}</div>
              <span className='text-black-50 text-center'>{currentUser?.roles?.[0]?.name}</span>
            </div>
            <Divider className='w-100 mt-2' />
            <ListItem button>My Account</ListItem>
            <ListItem button>Profile settings</ListItem>
            <ListItem button>Active tasks</ListItem>
            <ListItem button style={{ color: '#f83245' }} onClick={() => handleLogout()}>
              Log out
            </ListItem>
          </List>
        </div>
